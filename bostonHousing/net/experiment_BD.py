...
import net
import os
file_path = 'results/Log.txt'
os.remove('results/Log.txt')
os.remove('results/test_LL.txt')
os.remove('results/test_rmse.txt')
os.remove('results/test_MC_rmse.txt')
os.remove('results/time.txt')
...
index_features = index_features.astype(int) # convert ndarray to int

index_target = np.loadtxt('../data/index_target.txt')
index_target = index_target.astype(int)
...
# Starting to loop over the splits
